 ...
 *   @desc:     { 在制品零头产品 }
 */
module.exports.getVInventory = () =>
  axios({
    url: "/950/59140e2688.json",
  });

/** NodeJS服务端调用：
 *
 *   @database: { 质量信息系统 }
 *   @desc:     { 号票2号库装箱零头任务是否执行 }
 */
module.exports.getPrintWmsLogPackage = () =>
  axios({
    url: "/951/11740ea6ed.json",
  });
/** NodeJS服务端调用：
*
*   @database: { MES系统_生产环境 }
*   @desc:     { 批量记录装箱零头数据 } 
	以下参数在建立过程中与系统保留字段冲突，已自动替换:
	@desc:批量插入数据时，约定使用二维数组values参数，格式为[{prod_name,uncomplete,rec_time }]，数组的每一项表示一条数据*/
module.exports.addCbpcPackageUncomplete = (values) =>
  axios({
    method: "post",
    data: {
      values,
